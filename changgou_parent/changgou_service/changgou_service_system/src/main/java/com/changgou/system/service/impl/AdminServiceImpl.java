...
public class AdminServiceImpl implements AdminService {
    ...
    public boolean login(Admin admin) {
        //根据登录名获取管理员信息
        Admin admin1 = new Admin();  //加密前的密码
        admin1.setLoginName(admin.getLoginName());
        admin1.setStatus("1");
        Admin adminResult = adminMapper.selectOne(admin1);//加密后的密码

        //(加密前的密码 + adminResult.salt()) == adminResult.getPassword()
       /* String password = "123456";  //可逆加密算法  可以通过密钥 解密获得原来的数据
        //对称加密    AXUCE  token (密钥)
        123456 + AXUCE =  52847464e5d1186bb99f892e2a6d75fd ;
        //非堆成加密   公钥（所有都可以有的   ）    私钥 （服务器自己一个人用户）
        公钥 可以进行数据加密     私钥可以进行数据解密     公钥 --- 私钥  成对出现的*/
        if (adminResult == null){
            return false;
        }else{
            //对密码进行校验
